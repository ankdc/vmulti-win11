#include "vmulti.h"
#include <wdmsec.h> // For SDDL_* macros
    ...
    )
{
    ...
    // https://msdn.microsoft.com/en-us/library/windows/hardware/hh920402
    //
    ...
    )
{
    ...
    // Temporarily commenting out security descriptor to test if it causes loading issues
    /*
    //
    // Set security attributes to allow non-admin access
    // Using well-known string for System and Admin access
    //
    DECLARE_CONST_UNICODE_STRING(sddlString, L"D:P(A;;GA;;;SY)(A;;GA;;;BA)"); // System and Admin
    ...
    if (!NT_SUCCESS(status)) 
    {
        VMultiPrint(DEBUG_LEVEL_ERROR, DBG_PNP,
            "WdfDeviceInitAssignSDDLString failed 0x%x\n", status);
        return status;
    }
    */
    ...
    //
    // Register device interface for this device
    // but delay enabling it until all initialization is complete
    //
    ...
    // Now that all initialization is complete, enable the device interface
    // so that applications can find it
    WdfDeviceSetDeviceInterfaceState(device, &GUID_DEVINTERFACE_HID, NULL, TRUE);
    return status;
}
    ...
    )
{
    ...
    // Log report descriptor details for diagnostic purposes
    VMultiPrint(DEBUG_LEVEL_INFO, DBG_IOCTL,
        "VMultiGetReportDescriptor: Returning descriptor of size %d bytes\n", 
        (ULONG)bytesToCopy);

    // Log first few bytes to verify vendor collections
    if (bytesToCopy > 20) {
        VMultiPrint(DEBUG_LEVEL_INFO, DBG_IOCTL,
            "First bytes: %02x %02x %02x %02x %02x %02x %02x %02x\n",
            DefaultReportDescriptor[0], DefaultReportDescriptor[1],
            DefaultReportDescriptor[2], DefaultReportDescriptor[3],
            DefaultReportDescriptor[4], DefaultReportDescriptor[5],
            DefaultReportDescriptor[6], DefaultReportDescriptor[7]);
    }
